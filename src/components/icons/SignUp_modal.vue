    ...
    <div class="container" v-bind:class="{ hidden: !dataStore.showSignUp }">
        ...
        <form class="form">
            ...
            <div class="form-group">
                ...
                <p>Pays/Région</p>
                <select name="country-code" id="country-code" v-on:change="dataStore.countryCodeChange($event)">
                    <option value="none" selected disabled hidden>Pays/Régions</option>
                    <option v-for="countryCode in dataStore.countryCodes" v-bind:value="countryCode.callingCodes">
                        {{ ` ${countryCode.name}(+${countryCode.callingCodes})` }}
                    </option>
                </select>
                <span>
                    <span>{{ dataStore.registerInfo.countryCode }}</span>
                    <input type="tel" id="mobile" v-bind:placeholder="dataStore.countryCodePlaceholder" required
                        autocomplete="mobile" v-model="dataStore.registerInfo.mobile">
                    <span v-if="dataStore.registerInfo.mobile">
                        <span v-if="dataStore.mobileVerification">
                            <img src="@/assets/icons/circle-check-solid.svg" alt="logo de vérification">
                        </span>
                        <span v-else>
                            <img src="@/assets/icons/circle-exclamation-solid.svg" alt="logo de vérification">
                ...
                <p class="error-message">{{ dataStore.error.mobileError }}</p>
            </div>
            <div class="form-group">
